...
```{r}
hist(rbeta(1000,shape1=500,shape2=500))
...
prior=dbeta(p,shape1=500,shape2=500)
likelihood=dbinom(x,size=n,p=p)
...
```{r}
hist(rbeta(1000, shape1=5,shape2=200))

qbeta(c(0.245, 0.755), shape1 = 5, shape2 = 200)
...
```{r}
p=seq(from=0,to=1,by=0.0001)
prior=dbeta(p,shape1=5,shape2=200)
likelihood=dbinom(x,size=n,p=p)
...
```{r}
hist(rgamma(10000, shape = 4, rate = 1))
hist(rgamma(10000, shape = 12, rate = 3))
hist(rgamma(10000, shape = 120, rate = 30))
```
...
t=table(brfss$fruit_per_day)
x=sum(t*as.numeric(names(t)))
...
```{r q13}
# type your code for Question 12 here, and Knit
qgamma(c(0.05, 0.95), shape = a, rate = b)
```
