...
```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.075, alternative = "twosided", beta_prior=c(1,1))
```
...
```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.075, alternative = "twosided", hypothesis_prior = c(H1 = 0.3, H2 = 0.7), beta_prior=c(1,1))
```
...
```{r}
# comparing proportion of low-birth-weight among smokers and non-smokers

# hypothesis 1, there is no difference p_smokers == p_non_smokers
# pooled proportion, pooled prior
# prior p ~ beta(0.5,0.5)

# hypothesis 2, there is difference p_smokers != p_non_smokers
# prior p ~ beta(0.5,0.5) * beta(0.5,0.5)

bf_num = ( beta(92+18+1,781+108+1)/beta(1,1) )
bf_den = ( (beta(92+0.5,781+0.5)*beta(18+0.5,108+0.5))/(beta(0.5,0.5)*beta(0.5,0.5)) )
bf = bf_num/bf_den
#H1/H2
bf
```
...
```{r}
table(nc$premie, nc$habit)
```

```{r}
# comparing proportion of premature among smokers and non-smokers
...
bf_num = ( beta(739+107+1,133+19+1)/beta(1,1) )
bf_den = ( (beta(739+0.5,133+0.5)*beta(107+0.5,19+0.5))/(beta(0.5,0.5)*beta(0.5,0.5)) )
bf = bf_num/bf_den
#H1/H2
bf
```
