...
plot(density(m_wage_educ$residuals))
```
...
lwage_bas = bas.lm(lwage ~ iq, data = wage,
                    prior="BIC",
                    bestmodel=c(1,1),
                    n.models=1)

confint(coefficients(lwage_bas), level = 0.95)
```
...
```{r z-iq}
# type your code for the Exercise here, and Knit
wage$siq = (wage$iq-mean(wage$iq))/sd(wage$iq)
lwage_bas = lm(lwage ~ siq, data = wage) 
summary(lwage_bas) 
coefficients(lwage_bas) 
confint(lwage_bas)
```
...
coefficients(m_lwage_full)
confint(m_lwage_full)
```
...
m_lwage= lm(lwage ~ . -wage -sibs, data = na.omit(wage))
BIC(m_lwage) #581
m_lwage= lm(lwage ~ . -wage -feduc, data = na.omit(wage))
BIC(m_lwage) #580
m_lwage= lm(lwage ~ . -wage -meduc, data = na.omit(wage))
BIC(m_lwage) #582
```
...
AIC_lwage = stepAIC(m_lwage_full)
```
...
bma_lwage_red = bas.lm(lwage ~ . -wage, data = na.omit(wage_red),
                   prior = "JZS", 
