...
# p(M1 bad |WIN on m1)
numerator = p_win_m1_bad * p_m1_bad # Likelihood or p(data/model) * p(model)
denominator = p_win_m1_bad * p_m1_bad + p_win_m1_good * p_m1_good # Prior or p(data)
...
```{r}
# From previous result, we update the m1
p_m1_bad = 0.4
p_m1_good = 1 - p_m1_bad

p_m2_good = 1 - p_m1_good
p_m2_good
```
...
```{r}
data.frame(machine=1L, outcome="W")
```
...
```{r}
bandit_posterior(data2, prior=bandit_posterior(data1, prior=c(.5,.5)))
```
...
bandit_posterior(data1, prior=c(.5,.5)) %>% bandit_posterior(data2, prior=.)
```
...
data1 = data.frame(machine=c(1L,1L), outcome=c("W","W"))
data2 = data.frame(machine=c(2L,2L,2L), outcome=c("W","W","L"))
bandit_posterior(data2, prior=c(.5,.5)) %>% bandit_posterior(data1, prior=.)
```
